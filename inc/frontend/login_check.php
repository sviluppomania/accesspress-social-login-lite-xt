    ...
    function facebookLogin() {
        ...
        function GoogleLogin() {
            ...
            elseif( isset( $_GET['code'] ) ) { // Perform HTTP Request to OpenID server to validate key
                ...
                // Checks the state variables
                if( isset($_GET['state']) && $_GET['state'] !== '' ) {
                    parse_str( base64_decode( sanitize_text_field($_GET['state']) ), $state_vars );
                    if ( isset( $state_vars['redirect_to'] ) && $state_vars['redirect_to'] !== '') {
                        $encoded_url = sanitize_text_field($state_vars['redirect_to']);
                        $this->redirect( $redirect_uri . "&redirect_to=$encoded_url" );
                        die();
                    }
                }
